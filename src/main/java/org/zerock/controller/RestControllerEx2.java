...
public class RestControllerEx2 {
	...
	@RequestMapping("/ex2")
	public Rest1 method2() { //rest라는 객체 return
		
		//전송, 수신 방법이 http(HyperText Transper Protocl)
		log.info("method2");
...
//		String res = "이름:" + r.getName() + ", " + "나이:" + r.getAge();
	...
	@RequestMapping("/ex4")
	public Rest1 method4() { //.json으로 쓰면 json표기법으로 리턴
	...
	//본문의 타입 : String, ResponseEntity = 내가 원하는 대로 SC와 HEADER을 작성하고 싶을때 사용
		return ResponseEntity.status(200).body("hello"); //body에 본문을 넣어줌
	}
	...
	public ResponseEntity<String> method11(int num){ 
		if(num > 0) {
			...
			//localhost:8080/rest2/ex11?num=1
		}
	}
}
